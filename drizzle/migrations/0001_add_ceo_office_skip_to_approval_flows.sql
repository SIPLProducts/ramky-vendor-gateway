ALTER TABLE public.buyer_approval_flows
  ADD COLUMN IF NOT EXISTS skip_ceo_office boolean NOT NULL DEFAULT false;
...
BEGIN
  ...
  IF v_has_flow THEN
    ...
    IF v_flow.finance_2_user_id IS NOT NULL AND NOT v_flow.skip_finance_2 THEN
      ...
      v_count := v_count + 1;
    END IF;
    IF v_flow.ceo_office_user_id IS NOT NULL AND v_msme AND NOT v_flow.skip_ceo_office THEN
      v_level := v_level + 1;
      INSERT INTO public.vendor_approval_progress (vendor_id, level_id, level_number, status, stage)
