...
BEGIN
  ...
  -- Resolve inviting buyer + invitation tenant (latest invitation linked to this vendor).
  ...
  -- Fallback: if the vendor's saved tenant has no approval matrix configured but
  -- the invitation has a tenant that does, use the invitation tenant for routing.
