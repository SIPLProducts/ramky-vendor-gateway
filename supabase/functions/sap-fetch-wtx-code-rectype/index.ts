// Fetches "Fetch_Withholding_WTX_Code_REC_Type" SAP API config and returns
// two normalized arrays extracted from the upstream response:
...
const DEFAULT_CONFIG_NAME = "Fetch_Withholding_WTX_Code_REC_Type";
...
function pickCI(obj: any, key: string): any {
...
function findArrayByKey(data: any, keyName: string): any[] {
  if (!data) return [];
  if (Array.isArray(data)) {
    // Might be a mixed array. Filter based on presence of key fields later.
    return data;
  }
  // Direct key match (case-insensitive)
  for (const k of Object.keys(data)) {
    if (k.toLowerCase() === keyName.toLowerCase() && Array.isArray(data[k])) return data[k];
  }
  // Nested d.results
...
serve(async (req) => {
  ...
  try {
    ...
    // Extract TAXCODE and RECTYPE arrays.
    const taxcodeRaw = findArrayByKey(data, "TAXCODE");
    const rectypeRaw = findArrayByKey(data, "RECTYPE");

    const taxcodes = (taxcodeRaw || [])
      .map((r: any) => ({
        LAND1: String(pickCI(r, "LAND1") || "").trim().toUpperCase(),
        WITHT: String(pickCI(r, "WITHT") || "").trim(),
        WT_WITHCD: String(pickCI(r, "WT_WITHCD") || "").trim(),
        TCDESC: String(pickCI(r, "TCDESC") || "").trim(),
      }))
      .filter((r: any) => r.WT_WITHCD);

    const rectypes = (rectypeRaw || [])
      .map((r: any) => ({
        LAND1: String(pickCI(r, "LAND1") || "").trim().toUpperCase(),
        WITHT: String(pickCI(r, "WITHT") || "").trim(),
        QSREC: String(pickCI(r, "QSREC") || "").trim(),
        RCTXT: String(pickCI(r, "RCTXT") || "").trim(),
      }))
      .filter((r: any) => r.QSREC);

    return new Response(
      JSON.stringify({ success: true, taxcodes, rectypes, raw: data }),
      { headers: { ...corsHeaders, "Content-Type": "application/json" }, status: 200 },
    );
  } catch (e: any) {
    return new Response(
