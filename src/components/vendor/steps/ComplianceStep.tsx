...
import { FileUp, Award, Globe } from 'lucide-react';
...
export function ComplianceStep({
  ...
  // GST verified payload → fill extended fields
  const handleGstVerified = (d: Record<string, any>) => {
    if (d.legal_name) setValue('gstConstitutionOfBusiness', d.constitution_of_business || '');
    if (d.principal_place_of_business)
      setValue('gstPrincipalPlaceOfBusiness', d.principal_place_of_business);
    if (d.address && !d.principal_place_of_business)
      setValue('gstPrincipalPlaceOfBusiness', d.address);
  ...
  return (
    <form id="step-form" onSubmit={handleSubmit(handleFormSubmit)} className="space-y-6">
      ...
      {/* GST extended details (auto-populated from verification) */}
      ...
      {/* MSME category (auto-filled from verification) */}
