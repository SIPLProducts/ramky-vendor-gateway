...
export function PanKycTab(props: PanKycTabProps) {
  ...
  const handleVerify = async (extracted: Record<string, any>) => {
    ...
    const apiName = String(extracted.full_name || extracted.holder_name || '').trim();
    ...
    return { ok: true, message: `PAN verified — ${apiName || extractedPan || 'see API response below'}`, apiData: extracted };
  };
