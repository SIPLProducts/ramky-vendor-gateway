...
export function InlineFieldEditor({ tenantId, stepKey, field, defaultOrder = 1, onClose, builtInMode, builtInDefaults }: Props) {
  ...
  return (
    <div className="border rounded-lg p-4 bg-muted/30 space-y-3">
      {builtInMode && (
        <div className="flex items-start gap-2.5 rounded-md border border-primary/20 bg-primary/5 px-3 py-2">
          <Info className="h-4 w-4 text-primary shrink-0 mt-0.5" />
          <p className="text-xs text-foreground/90 leading-relaxed">
            Editing built-in field
            {builtInDefaults?.display_label && (
              <> <span className="font-semibold">"{builtInDefaults.display_label}"</span></>
            )}
            {' '}— field key &amp; type are locked. Click the{' '}
            <span className="inline-flex items-center justify-center h-3.5 w-3.5 rounded-sm border border-primary/30 bg-primary/10 text-primary align-text-bottom">
              <Info className="h-2.5 w-2.5" />
            </span>{' '}
            icon on the field row above to see its usage and why it matters.
          </p>
        </div>
      )}
