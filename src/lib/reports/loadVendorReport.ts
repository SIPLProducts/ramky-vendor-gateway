...
export async function loadVendorReports(filters: ReportFilters): Promise<VendorReportRow[]> {
  ...
  let q = supabase
    .from('vendors')
    .select(isSingle ? '*' : 'id, reference_number, legal_name, trade_name, account_holder_name, pan, pan_holder_name, vendor_type, status, created_at, submitted_at, primary_email')
