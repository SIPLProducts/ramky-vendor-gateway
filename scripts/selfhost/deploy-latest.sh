...
verify_ceo_skip_schema() {
  local present
  present=$(docker compose -f "$COMPOSE_FILE" exec -T db \
    psql -U supabase_admin -d postgres -tAc \
    "SELECT 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='buyer_approval_flows' AND column_name='skip_ceo_office'" \
    2>/dev/null | tr -d '[:space:]')
  if [[ "$present" != "1" ]]; then
    echo "ERROR: required database column buyer_approval_flows.skip_ceo_office is missing." >&2
    echo "Re-run this deployment without --skip-migrations before deploying the frontend." >&2
    exit 1
  fi
...
if [[ $SKIP_MIG -eq 0 && -d "$SOURCE_DIR/supabase/migrations" ]]; then
  ...
  # This additive migration is generated by the managed migration system.
  # Give it a timestamped self-host name so the existing runner applies it.
  CEO_SKIP_MIGRATION="$SOURCE_DIR/drizzle/migrations/0001_add_ceo_office_skip_to_approval_flows.sql"
  if [[ -f "$CEO_SKIP_MIGRATION" ]]; then
    cp -f "$CEO_SKIP_MIGRATION" "$MIG_DIR/20260922065300_add_ceo_office_skip_to_approval_flows.sql"
  else
